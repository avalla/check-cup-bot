 ...
 * e altrimenti tratta il dato come path/URL, quindi va convertito.
...
async function reserve({ chatId, cf, ricetta, maxDays = 30, zipFilter = '101[0-9][0-9]', addressFilter = '.*' }) {
  ...
  try {
    ...
    const checkAndClickSelector = async (selector, counter = 0) => {
      ...
      if (warning && counter < MAX_WARNING_RETRIES) {
    ...
    const { images: _images, ...printable } = result;
    ...
    result.images.push(await screenshot(page));
    // Note
    const presaVisionePresa = await (async function seeNotes(counter = 0) {
      ...
      if (counter >= MAX_NOTES_RETRIES) {
        return false;
      }
      return await seeNotes(counter + 1);
    })();
    if (!presaVisionePresa) {
      console.log(`${ricetta} Non ho trovato la conferma presa visione`);
      result.error = 'Non ho trovato il pulsante di conferma presa visione';
      return result;
    }
    ...
    await browser.close().catch((error) => console.error(`${ricetta} errore chiusura browser`, error));
